...
#[component]
pub fn DateInput<G: Html>(cx: Scope, s: (String, Value)) -> View<G> {
    ...
    view! { cx,
            div (class="mb-3 needs-validation") {
              ...
              input( on:blur=move|_| handle_blur(),  bind:value=sample_data,
                     class="form-control", type="date", placeholder=default_value.clone(), required=is_required, pattern="", min=minimal.unwrap(), max=maximal.unwrap(), step=steps.unwrap(), size="",
